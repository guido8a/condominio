...
<head>
    ...
    <title>Pagos Pendientes totales</title>

    <rep:estilos orientacion="p" pagTitle="${"Pagos pendientes totales"}"/>
        ...
        <g:each in="${res}" var="fila" status="j">
            ...
            <g:if test="${anterior == nuevo}">
                <g:if test="${nuevo == ultimo && (cont-1) == j}">
                    ...
                    <tr style="font-weight: bold;" class="azul"><td colspan="2" class="derecha">TOTAL:</td><td class="derecha">${total}</td></tr>
                </g:if>
                <g:else>
                    <tr style="width: 100%">
                        <td class="izquierda" style="width: 8%">${fila.depart}</td>
