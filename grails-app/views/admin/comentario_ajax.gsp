...
<label style="margin-left: 15px">Estado seleccionado: </label> <span style="color: ${estado == 'R' ? '#58d05c' : (estado == 'C' ? '#ff983b' : estado == 'B' ? '#af1627' : '#9a53af')}">${estado == 'R' ? 'REVISADO' : (estado == 'C' ? 'CORREGIR VALOR' : 'BORRAR')}</span>

<g:textArea name="comentario_name" id="comentarioIngreso" class="form-control" style="resize: none" maxlength="255"/>